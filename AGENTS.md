...
In-process: `ScreenTests.testScrollRegionParseCost` / `testPrintRunCost` / `testEditorRedrawParseCost` / `testSyncHoldParseCost` stderr. Release `y\n` 200k alt should stay ~5ms; 1 MiB `y\n` ~16ms; 10k full-width lines ~1ms; 20× 1 MiB alt CUP+SGR+print+EL ~16ms. A 2× jump on those is a regression — fix before commit. DEC 2026 hold-parse of 1 MiB `y\n` is not a ship canary (body waits for ESU).
